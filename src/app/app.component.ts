...
export class AppComponent implements AfterViewInit {
  ...
  public expandSection(index, evt) {
    ...
    const expandElement =
      evt.target.parentElement.parentElement.parentElement.parentElement.querySelector(
        ".card-body"
      );
  ...
  public chnageFilter(event) {
    ...
    const SortByName = (x, y) => {
      if (this.selectedOpt === "city") {
        return x.address[this.selectedOpt] === y.address[this.selectedOpt]
          ? 0
          : x.address[this.selectedOpt] > y.address[this.selectedOpt]
          ? 1
          : -1;
      } else {
        return x[this.selectedOpt] === y[this.selectedOpt]
          ? 0
          : x[this.selectedOpt] > y[this.selectedOpt]
          ? 1
          : -1;
      }
    };
